...
class SACIoUBboxLoss(BboxLoss):
    """尺度自适应 CIoU 损失 (Scale-Adaptive CIoU Loss)。

    针对无人机航拍场景，小目标数目众多且特征稀疏，标准 CIoU 损失对各尺度对象一视同仁，
    导致大目标占用过多回归梯度。SACIoU 通过面积权重使小目标在回归时获得更高梯度，
    缓解尺度不平衡带来的回归偏移问题。

    权重公式: w = 1 + lambda * (1 - normalized_area)
    其中 normalized_area = bbox_area / (stride^2)，范围 [0, 1]。

    参数:
        reg_max (int): DFL 正则化最大值。
        saciou_lambda (float): 尺度权重系数 (0=禁用, 推荐 1.0~3.0)。
    """
    ...
    def forward(
        ...
        """计算 SACIoU 损失：在标准 CIoU 基础上加入面积权重。"""
        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
...
class v8DetectionLoss:
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        # 更新 assigner 的图像尺寸 (用于 SGLA 尺度引导标签分配)
        if hasattr(self.assigner, "img_size"):
            self.assigner.img_size = int(imgsz.max().item())
