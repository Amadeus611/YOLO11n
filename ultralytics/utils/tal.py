...
class TaskAlignedAssigner(nn.Module):
    ...
    Attributes:
        ...
        scale_aware_boost (float): 尺度引导增益系数 (0=禁用, 0.5=适中, 1.0=较强)。
        img_size (int): 输入图像尺寸，用于计算归一化面积阈值。
    """
    ...
    def __init__(
        ...
        Args:
            ...
            scale_aware_boost (float, optional): 尺度引导增益系数，0 表示禁用。
        """
        super().__init__()
        self.topk = topk
        ...
        self.img_size = 640  # 默认值，由 loss 函数在训练时更新
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        # 尺度引导增益 (SGLA): 提升小目标的对齐度量，使其更容易被选为正样本
        if self.scale_aware_boost > 0:
            gt_wh = gt_bboxes[..., 2:] - gt_bboxes[..., :2]  # (b, n_max_boxes, 2)
            gt_area = (gt_wh[..., 0:1] * gt_wh[..., 1:2]).clamp_(min=1.0)  # (b, n_max_boxes, 1)
            # 归一化面积: 相对于 stride[0]^2 (最小特征图网格面积)
