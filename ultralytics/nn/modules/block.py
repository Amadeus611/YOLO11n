...
class SCAA(nn.Module):
    """Scale-Context Attention Aggregation — 跨尺度上下文注意力模块。

    接收来自两个不同尺度分支的特征，通过通道注意力实现跨尺度信息融合。
    用于增强小目标特征表达，将高层语义信息注入低层高分辨率特征。
    内置残差连接 (x_shallow + x_deep * attn)，自动处理不同空间尺寸的输入。

    参数:
        c1 (int): 输入通道数 (由 parse_model 自动计算为两个分支拼接后的总通道数)。
        c2 (int): 输出通道数 (与浅层分支通道数一致)。
    """

    def __init__(self, c1: int, c2: int):
        super().__init__()
        self.c2 = c2
        c_deep_in = c1 - c2  # 深层分支输入通道数
        # 深层特征通道压缩到 c2 (与浅层输出一致)
        self.cv_deep = Conv(c_deep_in, c2, 1, 1)
        # 拼接后的通道压缩 (输入: 浅层c2 + 压缩后深层c2 = 2*c2)
        self.cv_cat = Conv(2 * c2, c2, 1, 1)
        # 通道注意力: 全局池化 → 1×1 Conv → Sigmoid
        self.fc = nn.Conv2d(c2, c2, 1, 1, bias=True)
        # 输出投影
        self.cv_out = Conv(c2, c2, 1, 1)
    ...
    def forward(self, x: list | torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: 输入特征。支持两种格式:
               - list[Tensor]: [浅层特征 (高分辨率), 深层特征 (低分辨率)]，框架多源输入格式
               - Tensor: 已拼接的特征 [B, c1, H, W]

        Returns:
            增强后的特征张量 [B, c2, H, W]，空间尺寸与浅层特征一致。
        """
        # 处理框架传入的多源 list 格式
        if isinstance(x, list):
            x_shallow, x_deep = x[0], x[1]
        else:
            x_shallow, x_deep = x.split([self.c2, x.shape[1] - self.c2], dim=1)

        # 如果深层特征空间尺寸不同，上采样到浅层尺寸
        ...
        # 拼接 [浅层c2, 压缩后深层c2] 并压缩 (用于注意力计算)
        x_cat = self.cv_cat(torch.cat([x_shallow, x_deep], dim=1))
        # 通道注意力权重: sigmoid(fc(gap(x_cat)))
        attn = torch.sigmoid(self.fc(F.adaptive_avg_pool2d(x_cat, 1)))
        # 加权融合 + 残差连接
        out = x_shallow + x_deep * attn
